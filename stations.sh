...
if [ "$suffix" = "mp3" ] ; then
   ...
   #echo "$result"
   if [ "$suffix" = "pls" ] ; then
      wget -q $k -O- | grep "File1=" | sed -e 's/^File[1]*=//' | tr -d "\r" >> stations.lst # | mpc add
   elif [ "$suffix" = "m3u" ] ; then
      wget -q $k -O- | cat | xargs -r print # >> stations.lst # | xargs -r mpc add
      print "###"
      wget -q $k -O-
      print "==="
   else
      echo $k >> stations.lst # mpc add $k
   fi
fi
done
